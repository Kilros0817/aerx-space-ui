...
///Check if we can authenticate with pinata using provided keys
const authenticate = () => {
...
const pinToIPFS = async (content, content_name, content_type, username) => {
    const readableStreamForFile = fs.createReadStream(content);
    const options = {
        pinataMetadata: {
            name: `${content_name}`,
            keyvalues: {
                contentType: `AERX-${content_type}-NFT for ${username}`,
                owner: `${username}`
            }
        },
        pinataOptions: {
    ...
    pinata.pinFileToIPFS(readableStreamForFile, options).then((result) => {
        ...
        //Todo: Save state(or move the whole state for pinata to another file)
        //Todo: Alert user
...
export { pinToIPFS, getFileHash, unpinFromIPFS, authenticate}